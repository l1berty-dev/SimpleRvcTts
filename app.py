...
from SimpleRvcTts.config import Config
from SimpleRvcTts.lib.infer_pack.models import (
    SynthesizerTrnMs256NSFsid,
    SynthesizerTrnMs256NSFsid_nono,
    SynthesizerTrnMs768NSFsid,
    SynthesizerTrnMs768NSFsid_nono,
)
from SimpleRvcTts.rmvpe import RMVPE
from SimpleRvcTts.vc_infer_pipeline import VC
...
edge_output_filename = "SimpleRvcTts/edge_output.mp3"
tts_voice_list = asyncio.get_event_loop().run_until_complete(edge_tts.list_voices())
tts_voices = [f"{v['ShortName']}-{v['Gender']}" for v in tts_voice_list]

# model_root = "SimpleRvcTts/weights"
# models = [
...
def model_data(model_root, model_name):
...
rmvpe_model = RMVPE("SimpleRvcTts/rmvpe.pt", config.is_half, config.device)
...
def tts(
    ...
    try:
        ...
        tgt_sr, net_g, vc, version, index_file, if_f0 = model_data(model_root=model_root, model_name=model_name)
        t0 = time.time()
        if speed >= 0:
            speed_str = f"+{speed}%"
        else:
        ...
        times = [0, 0, 0]
        # voice1 = TTS('timur', 0, 'ru-RU-DmitryNeural-Male', 0.0, 'rmvpe', 1, 0.33)
        ...
        # print(info)
        # sounddevice.play(audio_opt, tgt_sr)
        return (
            info,
            edge_output_filename,
            (audio_opt, tgt_sr),
